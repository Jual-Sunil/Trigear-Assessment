"""OpenRouter provider implementation for LLM-based email summarization.

Exposes Qwen 3 (and any other OpenRouter-hosted model) through OpenRouter's
OpenAI-compatible REST API using a plain synchronous ``httpx.Client``.  The
implementation mirrors :class:`GeminiProvider` exactly in structure so that
it can be swapped in transparently via the factory.
"""
...
# Seconds to wait for OpenRouter to return a complete response.
# Kept generous because open-source models on shared infrastructure can be
# slower than first-party APIs.
...
class OpenRouterProvider(BaseLLMProvider):
    ...
    * ``LLMProviderError``          – Any other unexpected failure.
    ...
    def summarize(self, request: SummaryRequest) -> SummaryResponse:
        """Generate a validated email summary via the OpenRouter API.
        ...
        try:
            ...
            logger.info(
                ...
                extra={
                    "response": json.dumps(raw_response)[:3000],
                },
            )
            text = self._extract_text(raw_response)
            logger.info(
                "openrouter_extracted_text",
                extra={
                    "content": text[:3000],
                },
            )
            ...
            # Re-raise domain exceptions produced by helpers unchanged so
            # callers receive the most specific type.
            raise
        except Exception as exc:
            logger.error(
                "openrouter_request_unexpected_error",
                extra={
                    "provider": OPENROUTER_PROVIDER_NAME,
                    "email_id": str(request.email_id),
                    "error": str(exc),
            ...
            raise LLMProviderError(
                "OpenRouter summarization failed due to an unexpected error.",
                cause=exc,
            ) from exc

    def health_check(self) -> bool:
        """Report whether the OpenRouter provider is reachable and configured.
        ...
        if self._client is None:
            logger.warning(
                "openrouter_health_check_skipped_no_client",
                extra={"provider": OPENROUTER_PROVIDER_NAME},
            )
            return False
        ...
        try:
            ...
            logger.debug(
                "openrouter_health_check_result",
                extra={
                    "provider": OPENROUTER_PROVIDER_NAME,
    ...
    def complete(self, system_prompt: str, user_prompt: str) -> str:
        """Send a raw prompt pair to OpenRouter and return the model's text output.
        ...
        try:
            ...
            raise LLMProviderError(
                f"OpenRouter HTTP request failed: {exc}",
                cause=exc,
            ) from exc
    ...
    def _assert_ready(self) -> None:
        ...
        """
        if self._client is None or not self._settings.openrouter_api_key.strip():
    ...
    def _build_client(self) -> httpx.Client | None:
        ...
        if not api_key:
            logger.warning(
                "openrouter_client_not_built_missing_api_key",
                extra={"provider": OPENROUTER_PROVIDER_NAME},
            )
            return None
    ...
    def _build_prompt(self, request: SummaryRequest) -> str:
        ...
        The prompt embeds all available email metadata so the model has
        enough context to produce an accurate, grounded summary.
    ...
    def _call_api(self, prompt: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                "openrouter_request_transport_error",
                extra={
                    "provider": OPENROUTER_PROVIDER_NAME,
                    "error": str(exc),
                },
    ...
    def _extract_text(self, response: dict[str, Any]) -> str:
        ...
        OpenRouter follows the OpenAI chat-completion schema::
    ...
    def _parse_json_payload(self, content: str) -> dict[str, Any]:
        ...
        """
        # Some models wrap their JSON in markdown code fences despite being
        # told not to.  Strip the fence before attempting to parse.
        ...
        try:
            ...
            logger.error(
                "openrouter_response_json_parse_failure",
                extra={
                    "provider": OPENROUTER_PROVIDER_NAME,
        ...
        if not isinstance(payload, dict):
            logger.error(
                "openrouter_response_json_not_object",
                extra={
                    "provider": OPENROUTER_PROVIDER_NAME,
    ...
    def _extract_summary(self, payload: dict[str, Any]) -> str:
        ...
        if not isinstance(summary, str) or not summary.strip():
            logger.error(
                "openrouter_response_missing_summary",
                extra={
                    "provider": OPENROUTER_PROVIDER_NAME,
